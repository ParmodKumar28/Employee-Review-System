...
import { employeeAssignedFeedbacks, feedbacksReceived, getpendingFeedbacks, getsuccessfulFeedbacks, submitFeedback, submittedFeedbacks } from "../model/feedback.repository.js";
...
export const getFeedbackReceived = async(req,res,next)=>{
    try {
        ...
        res.render('received-feedbacks', {employee: req.session.employee, receivedFeedbacks: receivedFeedbacks, givenFeedbacks: givenFeedbacks});
    } catch (error) {
        next(error);
    }
}
